import pandas as pd

class BookLover:
    
    def __init__(self, name, email, fav_genre, num_books = 0, book_list = pd.DataFrame({'book_name':[], 'book_rating':[]})):
        # initialization method
        self.name = name
        self.email = email
        self.fav_genre = fav_genre
        self.num_books = num_books
        self.book_list = book_list
        
    def add_book(self, book_name, rating):
        # add a book to book_list if the book is not already in, otherwise tell the user
        if not self.has_book(book_name):
            new_book = pd.DataFrame({'book_name': [book_name], 'book_rating': [rating]})
            self.book_list = pd.concat([self.book_list, new_book], ignore_index=True)
            self.num_books += 1
        else:
            print(book_name + " is already in the Book List!")
            
    def has_book(self, book_name):
        # check if a book is in book_list
        return book_name in self.book_list.book_name.values
    
    def num_books_read(self):
        # check num_books
        return self.num_books
    
    def fav_books(self):
        # return a slice of books in book_list with a rating > 3
        return self.book_list[self.book_list.book_rating > 3]
        